...
def hazmat_main():
    ...
    while not STATE["quit"]:
        ...
        STATE["hazmat_running"] = True
        util.write_state(STATE, JSON_PATH)

        if STATE["frame_count"] > last_frame_read:
            frame = cv2.imread(FRAME_PATH)

        STATE["hazmat_running"] = False
        util.write_state(STATE, JSON_PATH)
        ...
        if frame is not None:
            ...
            if STATE["run_hazmat"]:
                ...
                if STATE["clear_all_found"]:
                    all_found = []
                    STATE["clear_all_found"] = False
            ...
            cv2.imwrite(HAZMAT_FRAME_PATH, frame)
