...
def processScreenshot(img, val):
    ...
    # print(myDict)
    words = ["explosive", "blasting agent", "non flammable gas", "inhalation hazard", "infectious substance", "flammable solid", "flammable liquid", 
...
def main():
    ...
    while len(received) != expected:
        ...
        count += 1
        for i in received:
            allReceived.append(i)
        # allReceived.append(received)
        if count == len(threshVals):
            breakOrNot = True
            break
    if breakOrNot == False:
        print(f"the {expected} labels seen are:")
        for i in received:
            print(i)
    elif breakOrNot == True:
        #find trial with the most hazmat labels recorded
        # longest = findMax(allReceived)
        allReceived = list(set(allReceived))
